...
torch.serialization.add_safe_globals(
    [Actor, Critic, nn.Sequential, nn.Linear, nn.ReLU, nn.Tanh]
)
...
def render(height, width, state: State):
    ...
    cv2.putText(
        canvas, "Actor Policy", [10, 20], cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 0), 1
    )
    cv2.putText(
        canvas,
        "PID Controller",
...
while not done_demo:
    ...
    cv2.putText(
        ...
        f"{action:+.1f}",
        (
            int(state.left_pad_pos[0]),
            int(state.left_pad_pos[1]) - int(state.pad_size[1]),
        ),
        cv2.FONT_HERSHEY_SIMPLEX,
        0.5,
        (255, 200, 0),
